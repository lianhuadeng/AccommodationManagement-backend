...
public class BedServiceImpl extends ServiceImpl<BedMapper, Bed> implements IBedService {
    ...
    @Override
    public PageDTO<BedListVO> pageList(Integer pageNum, Integer pageSize, Long parkId, Long buildingId, Long floor, Long roomId) {
        // 1. 构造 MP 分页对象
        Page<BedListVO> page = new Page<>(pageNum, pageSize);

        // 2. 调用自定义的 Mapper 方法
        IPage<BedListVO> resultPage = bedMapper.pageList(page, parkId, buildingId, floor, roomId);

        // 3. 封装 DTO 并返回
        PageDTO<BedListVO> dto = new PageDTO<>();
    ...
    @Override
    public LocationVO getLocationByUserIdForApplication(Long userId) {
